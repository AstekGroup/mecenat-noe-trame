...
export class Natura2000Service {
  ...
  private readonly filePath = path.join(
    process.cwd(),
    'src/data/natura2000-simple.json',
  );
  ...
  async getGeoJSON() {
    ...
    try {
      this.logger.log(
        `Chargement des données Natura 2000 depuis ${this.filePath}`,
      );
      ...
      this.logger.error(
        `Erreur lors du chargement de Natura 2000: ${error.message}`,
      );
      // Tentative avec un chemin alternatif (si on est dans dist)
      try {
        const altPath = path.join(
          process.cwd(),
          'dist/data/natura2000-simple.json',
        );
        ...
        this.logger.error(
          `Échec du chargement alternatif: ${altError.message}`,
        );
